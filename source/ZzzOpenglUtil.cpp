...
#ifdef DO_PROCESS_DEBUGCAMERA
	if( g_pDebugCameraManager->GetCurrentActivity() != EDEBUGCAMERA_NONE )
	{
		gluPerspective2(CameraFOV,(float)Width/(float)Height,CameraViewNear,CameraViewFar*20.f);
		
		glMatrixMode(GL_MODELVIEW);
		glPushMatrix();
		glLoadIdentity();
		
		CDebugCameraInterface *pDebugCam = g_pDebugCameraManager->GetActiveDebugCamera();
		
		glRotatef( pDebugCam->m_vAngle[0], 1.0f, 0.0f, 0.0f );
		glRotatef( pDebugCam->m_vAngle[1], 0.0f, 1.0f, 0.0f );
		glRotatef( pDebugCam->m_vAngle[2], 0.0f, 0.0f, 1.0f );
		
		glTranslatef(-pDebugCam->m_vCameraPos[0],
			-pDebugCam->m_vCameraPos[1],
			-pDebugCam->m_vCameraPos[2]);
	}
	else
	{
...
void CreateScreenVector2( int sx, int sy, vec3_t Target, bool bFixView )
{
	vec3_t p1,p2;
...
{
	float ScreenRate_x = 1.0f;//(float)WindowWidth / 640;
	float ScreenRate_y = 1.0f;//(float)WindowHeight / 480;
	...
	glViewport2( static_cast<float>(pos.x * ScreenRate_x ), 
  				 static_cast<float>(pos.y * ScreenRate_y ), 
				 static_cast<float>(size.x * ScreenRate_x ), 
				 static_cast<float>(size.y * ScreenRate_y )
			   );

	gluPerspective2( 3.141592654f/4, static_cast<float>(size.x*ScreenRate_x)/static_cast<float>(size.y*ScreenRate_y), 20.f, 20000.f );
// -----------------------------------------------------------------------------------------------------//
	glMatrixMode(GL_MODELVIEW);
	glPushMatrix();
	glLoadIdentity();
...
// -----------------------------------------------------------------------------------------------------//
	...
	glTranslatef( -10000.0f, 0.0f, -65.f);
