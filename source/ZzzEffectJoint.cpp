...
extern float g_fBoneSave[10][3][4];	// ÀÓ½Ã ÀúÀåÀ» À§ÇÑ °ø°£

///////////////////////////////////////////////////////////////////////////////
// ±â´Ù¶õ ¿¬°áµÈ È¿°ú Ã³¸®(°ÔÀÓ»óÀÇ ¹ø°³ ¸¶¹ý µîµî)
///////////////////////////////////////////////////////////////////////////////
				 ...
				 WORD SkillIndex,WORD SkillSerialNum,int iChaIndex, const float* vPriorColor, short int sTargetindex)
{
	for(int i=0;i<MAX_JOINTS;i++)
	{
		...
		if(!o->Live )
		{
			...
			// subtype 47 Àº µ¥µé¸®½ºÅÂÇÁ ÀÜ»ó Àü¿ë(»ç¿ëÇÏÁö ¸»°Í)
			case BITMAP_JOINT_ENERGY:				
				...
				case 54:
					...
					break;
#ifdef LDK_ADD_14_15_GRADE_ITEM_SET_EFFECT
				case 55:	//¾ÆÀÌÅÛ ¾÷±×·¹ÀÌµå 14 15¿ë ¼ÂÆ®ÀÌÆåÆ®1
				case 56:	//¾ÆÀÌÅÛ ¾÷±×·¹ÀÌµå 14 15¿ë ¼ÂÆ®ÀÌÆåÆ®2
					{
						o->Velocity = 0.f;
						o->MaxTails = 8;
						o->LifeTime = 10;
...
#endif //LDK_ADD_14_15_GRADE_ITEM_SET_EFFECT
#ifdef LDK_ADD_EXTENSIONMAP_BOSS_MEDUSA_EYE_EFFECT
				case 57:	//¾ÆÀÌÅÛ ¾÷±×·¹ÀÌµå 14 15¿ë ¼ÂÆ®ÀÌÆåÆ®2
					{
						o->Velocity = 0.f;
				...
				if(!vPriorColor)
				{
					...
					case 46: Vector(0.1f, 0.25f, 0.1f, o->Light); break;	// ÃÊ·Ï 
#ifdef ADD_SOCKET_ITEM
					case 47: Vector(0.9f, 0.f, 0.f, o->Light); break;	// µ¥µéÀÌ½ºÅ×ÇÁ ÀÜ»ó(»¡°­)
#endif // ADD_SOCKET_ITEM
					}					
				}
			...
			case BITMAP_JOINT_HEALING:
                ...
                else if(o->SubType == 5) //  ´Ü¼øÈ÷ À§·Î ¿Ã¶ó°£´Ù.
                {
                    o->Scale += rand()%10-5;
			...
			case BITMAP_2LINE_GHOST:
				{
					...
					else if( o->SubType == 1 )		// ¸¶·ÂÁõ´ë(Èæ¸¶¹ý»ç) °í½ºÆ® È¿°ú
					{
						o->RenderType = RENDER_TYPE_ALPHA_BLEND;
						o->Scale = Scale;
				...
				case 24:	// ¼ÒÈ¯¼ö ¶ó±Û ²¿¸® ÀÌÆåÆ®.
					{
						...
						// ¶ó±Û ¸ðµ¨ »ý¼º.
						float fRargleScale = o->Scale / 70.f;	// ¸ðµ¨ Å©±â´Â ²¿¸® Å©±â¿¡ ºñ·Ê.
						if (0.9f <= fRargleScale)	// 0.9f ÀÌ»ó¸¸ ¶ó±Û ¸ðµ¨ »ý¼º.
						{
							vec3_t vLight;
			...
			case BITMAP_JOINT_SPARK:
				...
				case MODEL_FENRIR_SKILL_THUNDER:
					...
					else if(o->SubType == 2)	// ÆÄ¶û
					{
						o->TexType = BITMAP_JOINT_THUNDER;
						Vector ( 0.7f, 0.7f, 1.0f, o->Light );
					}
					else if(o->SubType == 3)	// ³ë¶û
					{
						o->TexType = BITMAP_JOINT_THUNDER;
						Vector ( 0.9f, 0.9f, 0.3f, o->Light );
					}
					else if(o->SubType == 4)	// °ËÁ¤
					{
					...
					else if(o->SubType == 6)	// ºí·ç
					{
						o->TexType = BITMAP_FLASH;
						Vector ( 0.2f, 0.3f, 1.0f, o->Light );
					}
					else if(o->SubType == 7)	// °ñµå
					{
						o->TexType = BITMAP_FLASH;
				...
				case BITMAP_JOINT_THUNDER:
					...
					case 3 :    //  Á¦ÀÚ¸®¿¡ ¿øÇüÀ¸·Î Àü±âÈ¿°ú·Î »ç¿ë.
					...
					case 24:
						...
						o->Velocity = 20.f;
						break;
					case 25:		// ¸ó½ºÅÍ ÁÖÀ§¿¡ Àü±â°¡ Áö±ÛÁö±Û
						{
							o->LifeTime = 20;
							o->MaxTails = 8;
							//o->Velocity = 50.f;
					...
					case 33:			// 16: ±â°£Æ½½ºÅè ¹ø°³¿Í µ¿ÀÏÇÏ³ª »ö¸¸ º¯°æ
            ...
            case BITMAP_JOINT_THUNDER+1:
                ...
                else if ( o->SubType==1 || o->SubType==2 || o->SubType==3 || o->SubType==5 || o->SubType==6 || o->SubType == 7 ) //  À§¿¡¼­ ¾Æ·¡·Î ¹ø°³°¡ ³»·ÁÄ£´Ù.
				{
                    o->MaxTails = MAX_TAILS;
                    o->LifeTime = 20;
                ...
                else if ( o->SubType==4 )   //  ½ÃÀÛÀ§Ä¡¿¡¼­ ¸ñÇ¥À§Ä¡·Î ¹ø°³°¡ ¿¬°áµÈ´Ù.
                {
                    o->MaxTails = MAX_TAILS;
                    o->LifeTime = 20;
                    o->m_bCreateTails = false;
                    o->byOnlyOneRender= 2;
                    o->TargetIndex[0] = (int)(o->MaxTails/1.5f);        //  Ã³À½ »ý¼ºÇÑ °í¸® °¹¼ö. ³ª¸ÓÁö´Â ´ÙÀ½¿¡ »ý¼º.
                    o->TargetIndex[1] = o->MaxTails-1;
                    o->StartPosition[0] = (float)( 1.f/o->MaxTails );   //  Ã³À½ À§Ä¡ °£°ÝÀ» À§ÇÑ ´Ü°è°ª.
			...
			case MODEL_SPEARSKILL:	// ¹æ¾î¸·
                ...
                case 3:
                    ...
                    o->TexType  = BITMAP_JOINT_SPIRIT;
                    break;
                case 5:     //  ¹èÆ²¸¶½ºÅÍÀÇ ¸¶¹ý ÇØÁ¦¿ë È¿°ú.
                case 6:     //  ¹èÆ²¸¶½ºÅÍÀÇ ¸¶¹ý ÇØÁ¦¿ë È¿°ú.
                case 7:     //  ¹èÆ²¸¶½ºÅÍÀÇ ¸¶¹ý ÇØÁ¦¿ë È¿°ú.
                    ...
                    if ( o->SubType==5 )        //  ¹°¸® ¹æ¾î·Â Çâ»ó.
                    {
						Vector ( 1.f, 1.f, 0.8f, o->Light );
						o->TexType  = BITMAP_FLARE+1;
                    }
                    else if ( o->SubType==6 )   //  °ø°Ý·Â Çâ»ó.
                    {
						Vector ( 1.f, 0.8f, 1.f, o->Light );
						o->TexType  = BITMAP_FLARE+1;
                    }
                    else if ( o->SubType==7 )   //  ¹æ¾î·Â Çâ»ó.
                    {
            ...
            case BITMAP_FLARE_BLUE:
                ...
                o->MaxTails     = 20;
                o->Scale        = 10.f;
                if( o->SubType==0 || o->SubType==18 )     //  ÀÚ½ÅÀÇ ÁÖÀ§¸¦ µ¹¸é¼­ ¿Ã¶ó°¡´Â ºûÁÙ±â. ( 18: ´Ù¸¥ ÅØ½ºÃÄ ).
                {
                ...
                else if ( o->SubType==20 )      //  ºÎ¸ðÀÇ À§Ä¡·Î µû¶ó°£´Ù.
                {
                    if ( o->Target==NULL )
                    {
                        o->Live = false;
                ...
                else if( o->SubType==10 )     //  ÀÚ½ÅÀÇ ÁÖÀ§¸¦ µ¹¸é¼­ ¿Ã¶ó°¡´Â ºûÁÙ±â. À§¿¡¼­ ¾Æ·¡·Î.
                {
                    o->Scale = Scale;
                    o->Direction[2] = (rand()%150)/100.f;
                    o->Direction[1] = (float)(rand()%500-250);
				...
				else if ( o->SubType==14 || o->SubType==15 )	//	µ¹´Ù°¡ »ç¶óÁü. ÀÓÆÑÆ® ÀÖ°Ô.
				{
                    o->RenderType = RENDER_TYPE_ALPHA_BLEND_OTHER;
                    o->Scale   = Scale;
                    o->TexType = BITMAP_JOINT_SPIRIT;
                ...
                else if ( o->SubType==16 )      //  ºû¿¡ »ý°å´Ù°¡ »ç¶óÁø´Ù.
                {
                    o->LifeTime = 60;
                    o->m_bCreateTails = false;
                ...
                else if ( o->SubType==17 )      //  ´ÙÅ©È¦½ºÀÇ ¹ß¿¡ ºÙÀÌ´Ù. ±âÀÇ Èå¸§.
                {
                ...
                else if(o->SubType==21)  //  Ä³¸¯ÅÍ ÁÖº¯¿¡¼­ ¿©·¯ °¡Áö ºû±ò·Î ¿Ã¶ó°£´Ù. ÀüÃ¼ È¿°ú
                {
                    o->Direction[2] = (float)(rand()%20+35);
                    o->Scale    = Scale;
				...
				else if(o->SubType==22) //   Ä³¸¯ÅÍ ÁÖº¯¿¡¼­ µû¶ó´Ù´Ï¸ç µ·´Ù.
				{
				...
				//ÀÚ½Å ÁÖÀ§¿¡¼­ Á¶¿ëÈ÷ °è¼ÓÇØ¼­ ¿Ã¶ó¿À´Â ºû
				...
				//Ä³¸¯ÅÍ ÁÖº¯À» µ¹¸é¼­ ·£´ýÀ¸·Î 41¹ø ÀÌÆåÆ®¸¦ °è¼Ó ¹ß»ý½ÃÅ²´Ù.
                ...
                else if ( o->SubType==19 )  //  À§¿¡¼­ ¾Æ·¡·Î ³»·Á¿À´Â ºû.
                {
                    o->Direction[2] = -(float)(rand()%20+35);
                    o->Scale    = Scale;
                    o->LifeTime = 25+rand()%50;
                ...
                else if(o->SubType==3)  //  ÀÚ½ÅÀÇ °¢µµ¿¡ Á÷¼± ÀÌµ¿ÇÏ´Â ºûÁÙ±â.
                {
                    o->Velocity = 50.f;
                    o->Scale    = Scale;
                    o->LifeTime = 5;
				...
				else if ( o->SubType == 4 || o->SubType==6 || o->SubType == 12)	// ²¿±ò¸ð¾ç
				{
					o->Scale = Scale;
					if ( o->SubType == 12)
					{
				...
				else if ( o->SubType == 7)	// ¼öÁ¤
				{
					Vector(0.2f,0.2f,1.0f,o->Light);
					o->MultiUse = rand()%10;	// ¾î¶² °ÍÀº ´õ »¡¸® ÅÍÁö°Ô ÇÏ±â
					o->LifeTime = 30 + o->MultiUse;	// ¾î¶² °ÍÀº ´õ »¡¸® ÅÍÁö°Ô ÇÏ±â
					o->MaxTails = 15;
				...
				else if ( o->SubType == 11 )	// È¦¸® »çÀÌÆ® º¸¿ì È­»ì È¿°ú.
				{
					Vector(0.2f,0.2f,1.0f,o->Light);
					o->MultiUse = 0;	// ¾î¶² °ÍÀº ´õ »¡¸® ÅÍÁö°Ô ÇÏ±â
					o->LifeTime = 30;	// ¾î¶² °ÍÀº ´õ »¡¸® ÅÍÁö°Ô ÇÏ±â
					o->MaxTails = 15;
					o->Direction[0] = ( float)( rand() % 3000);
					o->Scale        = 30.f;
				}
#ifdef CRYINGWOLF_2NDMVP
				else if(o->SubType==25)		//. ¹ß¶÷ °üÅë ½ºÅ³È¿°ú
				{
                ...
                else if(o->SubType==13)         //  À§·Î ¿Ã¶ó°¡´Â ºûÁÙ±â.(¿¬ÇÏ°í ÃµÃµÈ÷)
                {
                    o->Direction[2] = (float)(rand()%20+35);
                    o->Scale    = Scale;
                    o->LifeTime = 25+rand()%50;
				...
				else if ( o->SubType == 45 || o->SubType == 46)	// ¼öÁ¤
				{
					Vector(0.2f,0.2f,1.0f,o->Light);
					o->MultiUse = rand()%10;	// ¾î¶² °ÍÀº ´õ »¡¸® ÅÍÁö°Ô ÇÏ±â
					o->LifeTime = 30 + o->MultiUse;	// ¾î¶² °ÍÀº ´õ »¡¸® ÅÍÁö°Ô ÇÏ±â
				...
				//////////////////////////////////////////////////////////////////////////
					...
					case BITMAP_FLARE+1:    //  ²¿¿© ³ª°¡´Â ±¤¼±.
						...
						case 5://  ¿òÁ÷¿©¼­ Å¸ÄÏÀ§Ä¡·Î Ã£¾Æ ÀÌµ¿ÇÑ´Ù.  .  ( ¼ÓÀÌºó ¶ì )
							Vector ( 1.f, 1.f, 1.f, o->Light );
							o->RenderType = RENDER_TYPE_ALPHA_BLEND_OTHER;
							o->RenderFace = RENDER_FACE_TWO;
							o->LifeTime = 50;
						...
						case 6://  ¿òÁ÷¿©¼­ Å¸ÄÏÀ» Ã£¾Æ ¿òÁ÷ÀÎ´Ù. Å¸ÄÏÀº Object.  ( ¼ÓÀÌºó ¶ì )
							Vector ( 1.f, 1.f, 1.f, o->Light );
							o->RenderType = RENDER_TYPE_ALPHA_BLEND_OTHER;
							o->Scale    = Scale;
							o->MaxTails = 16;
						...
						case 8: //  ÀÚ½ÅÀÇ ºÎ¸ð¸¦ µû¶ó¼­ ¿òÁ÷ÀÎ´Ù. ( ³ôÀÌ °ªÀº °íÁ¤µÇ¾îÀÖ´Ù. )
						case 9: //  ÀÚ½ÅÀÇ ºÎ¸ð¸¦ µû¶ó¼­ ¿òÁ÷ÀÎ´Ù. 
							o->RenderFace = RENDER_FACE_TWO;
						case 10:
						case 11:
							o->RenderType = RENDER_TYPE_ALPHA_BLEND_OTHER;
						...
						case 12://  ¿òÁ÷¿©¼­ Å¸ÄÏÀ» Ã£¾Æ ¿òÁ÷ÀÎ´Ù. Å¸ÄÏÀº Object.  ( ¼ÓÀÌºó ¶ì )
...
//					Vector(0.f,1.f,0.f,o->Light);
				...
				else if( o->SubType==8 )	// SubType : 7 ¹ø ÂüÁ¶
                {
					o->LifeTime = 20;
                    o->m_bCreateTails = false;
					o->Scale    = Scale;
...
//					Vector(1.f,1.f,1.f,o->Light);
				...
				else if( o->SubType==20 )	// SubType : 7 ¹ø ÂüÁ¶
				{
					o->Scale    = Scale;
					o->Velocity = 8.f;
                    o->Direction[0] = 3.5f;
            ...
            case BITMAP_FLARE_FORCE:
                ...
                if ( o->SubType>=1 && o->SubType<=4 
					|| (o->SubType >= 11 && o->SubType <= 13)	//^ Ææ¸± ÀÌÆåÆ® °ü·Ã
					)
                {
                    o->TexType = BITMAP_JOINT_THUNDER;
                    o->Velocity = -3.f;
            ...
            case BITMAP_FLASH:
				...
				else if( o->SubType==6)
				{
					...
					o->LifeTime = 25;
					o->Weapon = o->LifeTime;
					o->Velocity = (float)(70 + rand()%3) * Q_PI / 180; //°íÀ¯ ¹ß»ç°¢
					...
					case 0:
						{
							...
							VectorCopy(TargetPosition, o->TargetPosition);
							o->Angle[0] += (float)((rand()%100)-50);				// ¹æÇâ : -140 ~ 140
							o->Angle[1] += (float)((rand()%100)-50);				// ¹æÇâ : -140 ~ 140
							o->Angle[2] += (float)((rand()%100)-50);				// ¹æÇâ : -140 ~ 140
							o->Velocity = (float)(1+((float)(rand()%10)*0.2f));		// ¼Óµµ : 20 ~ 40
							o->LifeTime = (float)(30+(rand()%20-10));				// ¶óÀÌÇÁÅ¸ÀÓ : 20 ~ 30
							o->Scale    = Scale+(float)((rand()%60-30));			// ½ºÄÉÀÏ : Scale-30 ~ Scale+30
							o->MaxTails = (float)(20+(rand()%10-5));				// Å×ÀÏ : 5 ~ 15
						}
						break;	
					}
				}
...
#ifdef ASG_ADD_SUMMON_RARGLE
			case 48:
				// ¾Ö´Ï¸ÞÀÌ¼ÇÀÌ ÀÖ´Â ¸ðµ¨¿¡¼­ º»À§Ä¡¸¦ ±¸ÇÏ±â À§ÇØ¼­´Â ¾Æ·¡¿Í °°ÀÌ
				//°­Á¦ÀûÀ¸·Î ¾Ö´Ï¸ÞÀÌ¼Ç ÇÔ¼ö¸¦ È£Ãâ.
				Models[o->Target->Type].Animation(BoneTransform, o->Target->AnimationFrame,
					o->Target->PriorAnimationFrame, o->Target->PriorAction, o->Target->Angle,
					o->Target->HeadAngle, false, false);
...
//					if(o->LifeTime%2 == 0)
						CreateParticle(BITMAP_SMOKE,o->Position,o->Angle,o->Light,59,1.0f);	// ¿¬±â
#else
					if(o->LifeTime%20 <= 9)
					{
						o->m_sTargetIndex = -10;
...
#ifndef CSK_EVIL_SKILL			// Á¤¸®ÇÒ ¶§ Áö¿ö¾ß ÇÏ´Â ¼Ò½º
				// ¾Ç·ÉÀÇ ¸ðµç ÆÇº°À» ¼­¹ö¿¡¼­ Ã³¸®ÇÏ°Ô²û ÇÏ±âÀ§ÇØ ¹Ø¿¡ ÄÚµå¸¦ Á¦°Å
				// Óäàëåí ïðèâåäåííûé íèæå êîä, ÷òîáû ñåðâåð ìîã îáðàáàòûâàòü âñþ èäåíòèôèêàöèþ çëûõ äóõîâ.
				if(o->LifeTime%15 == 0)
				{
					if(o->Target == &Hero->Object)
						AttackCharacterRange(o->Skill,o->Position,150.f,o->Weapon,o->PKKey,o->m_bySkillSerialNum);//Å¬¶óÀÌ¾ðÆ®¸¶¹ýÃ³¸® (Êëèåíòñêàÿ ìàãè÷åñêàÿ îáðàáîòêà)
				}
#endif // CSK_EVIL_SKILL		// Á¤¸®ÇÒ ¶§ Áö¿ö¾ß ÇÏ´Â ¼Ò½º
			}
	...
	case MODEL_SPEARSKILL:	// ¹æ¾î¸·
        ...
        else if ( o->SubType==5 || o->SubType==6 || o->SubType==7 )       //  ¹èÆ² ¸¶½ºÅÍ ¸¶¹ý ÇØÁ¦ È¿°ú
        {
            for ( int i=0; i<3; ++i )
            {
                o->Angle[2] += 10.f;
        ...
        else
		{
            ...
            if ( o->SubType==4 
				|| o->SubType == 9	//^ Ææ¸± ÀÌÆåÆ® °ü·Ã(¹æ¾î·ÂÇâ»ó)
				)
            {
                //  ¹æÇâÁß.
			...
			if ( 0 == o->SubType || o->SubType==4 || o->SubType == 9	//^ Ææ¸± ÀÌÆåÆ® °ü·Ã(¹æ¾î·ÂÇâ»ó)
#ifdef CSK_LUCKY_SEAL
				|| o->SubType == 10 || o->SubType == 11
#endif // CSK_LUCKY_SEAL
				|| o->SubType == 14	// ¼ÒÈ¯ ¼Õ¸ñ¸µ
				|| o->SubType == 16 // ¸¶¿ì½º Å¬¸¯ ÀÌµ¿ ÇÒ À§Ä¡ ÀÌÆÑÆ®
				)
			{
				for(int j=o->NumTails-1;j>=0;j--)
				{
	...
	case BITMAP_JOINT_THUNDER:
...
#endif // _VS2008PORTING
		{
			...
			case 24:		// ¸ó½ºÅÍ
				{
					// o : ¶óÀÌÆ®´× ¿ÀºêÁ§Æ®
					// o->Target : ÁÖÀÎ°ø ¿ÀºêÁ§Æ®
					if( o->Target )
					{
						if(j == 0)			// ????
						{	
							OBJECT* pSourceObj = o->Target;			// ÁÖÀÎ°ø ¿ÀºêÁ§Æ®
							CHARACTER* pTargetChar = &CharactersClient[FindCharacterIndex(o->m_sTargetIndex)];
							OBJECT* pTargetObj = &pTargetChar->Object;	// Å¸°Ù ¿ÀºêÁ§Æ®
							...
							VectorCopy( vPos, o->Position );						// ÁÖÀÎ°ø Æ÷Áö¼Ç
							VectorCopy( pTargetObj->Position, o->TargetPosition);	// ¸ó½ºÅÍ Æ÷Áö¼Ç
							
							// ¸ó½ºÅÍÀÇ ÁÂÇ¥¿¡¼­ À§·Î Á¶±Ý ¿Ã·ÁÁØ´Ù.
							o->TargetPosition[2] += 80.f;
						}
				...
				break;
			case 25:			// ¶óÀÌÆ®´×½ºÅ³¿¡ ¸ÂÀº Ä³¸¯ÅÍÀÇ ÀÌÆÑÆ® Áö±ÛÁö±Û
				{
					if ( o->Target )
					{
						VectorCopy(o->Target->Position, o->TargetPosition);
            ...
            else if ( o->SubType==4 )   //  Àü°Ý ¸¶¹ý ¾Æ·¡¿¡¼­ À§·Î.
            {
                o->Direction[0] = (float)(rand()%64-32);
            }
            else if ( o->SubType==5 )   //  Àü°Ý ¸¶¹ý À§¿¡¼­ ¾Æ·¡·Î.
            {
                o->Direction[0] = (float)(rand()%32-16);
            }
			else if ( o->SubType==6 )   //  º­¶ô.
            {
	...
	case BITMAP_JOINT_THUNDER+1:
        ...
        else if ( o->SubType==4 )   //  ½ÃÀÛÀ§Ä¡¿¡¼­ ¸ñÇ¥À§Ä¡·Î ¿¬°áµÇ´Â ¹ø°³.
        {
            if ( o->Target!=NULL )
            {
                VectorCopy ( o->Target->Position, o->Position );
                ...
                VectorSubtract ( o->TargetPosition, o->Position, Position );
                VectorScale ( Position, o->StartPosition[0], Position );        //  ¹Ýº¹µÈ ÀÌµ¿ °£°Ý Vector.
#ifdef _VS2008PORTING
                ...
                VectorSubtract ( o->TargetPosition, o->Position, Position );
                VectorScale ( Position, o->StartPosition[1], Position );        //  2¹øÂ° ¹Ýº¹µÇ´Â ÀÌµ¿ °£°Ý Vector.
#ifdef _VS2008PORTING
    ...
    case BITMAP_FLARE_BLUE:
...
#ifdef GUILD_WAR_EVENT
        else if(o->SubType == 21)    //  ¾Æ·¡¿¡¼­ À§·Î ¿Ã¶ó°¡´Â ºû. ÀüÃ¼ ¹üÀ§
        {
            if ( o->LifeTime<=25 )
            {
                o->Direction[2] += 5.f;
		...
		{
			//XYÆò¸é»óÀÇ ¹æÇâÀ» µû¶ó ½ºÇÁ¸µÃ³·³ ¿òÁ÷ÀÎ´Ù.
			...
			)	// ¼öÁ¤
		{
            ...
            if ( o->SubType==11 )   //  È¦¸® »çÀÌÆ® º¸¿ì È­»ì.
            {
...
#ifdef CRYINGWOLF_2NDMVP
			if ( o->SubType==25 )  // ¹ß¶÷¿ë °üÅë È¿°ú
			{
				Vector ( 0.9f, 0.4f, 0.6f, Light );
				if ( o->Skill==1 )
				{
					VectorCopy ( o->Position, o->Target->Position );		//. °üÅëÀ» À§ÇØ
					CheckClientArrow ( o->Target );
				}
			}
#endif // CRYINGWOLF_2NDMVP
    ...
    case BITMAP_FLARE+1:    //  ²¿¿© ³ª°¡´Â ±¤¼±.
        if ( o->SubType==5 )    //  ¿òÁ÷¿©¼­ Å¸ÄÏÀ§Ä¡·Î Ã£¾Æ ÀÌµ¿ÇÑ´Ù.  .  ( ¼ÓÀÌºó ¶ì )
        {
			if(o->LifeTime>=25)
			{
				o->Angle[2] -= 10.f;
        ...
        else if ( o->SubType==6 )   //  ¿òÁ÷¿©¼­ Å¸ÄÏÀ» Ã£¾Æ ¿òÁ÷ÀÎ´Ù. Å¸ÄÏÀº Object.  ( ¼ÓÀÌºó ¶ì )
        {
            if ( o->Target!=NULL )
            {
				if(o->LifeTime>=30)
        ...
        else if ( o->SubType==8 || o->SubType==9 )
        {
			...
			|| o->SubType == 18
#endif // ADD_SOCKET_ITEM
			)   //  ¿òÁ÷¿©¼­ Å¸ÄÏÀ» Ã£¾Æ ¿òÁ÷ÀÎ´Ù. Å¸ÄÏÀº Object.  ( ¼ÓÀÌºó ¶ì )
        {
            if ( o->Target!=NULL )
            {
                if ( o->Target->Live==false )
        ...
        if ( o->SubType==10 || o->SubType==8)
        {
...
#endif // _VS2008PORTING
            {
                if( o->NumTails<o->MaxTails-1)
                {
					...
					if ( o->SubType==8 )
					{
                        CreateJoint(BITMAP_JOINT_THUNDER,o->Position,o->Position,o->Angle,3,NULL,rand()%10+5.f,5,10); //  Àü±â 
                        CreateJoint(BITMAP_JOINT_THUNDER,o->Position,o->Position,o->Angle,3,NULL,rand()%8+4.f,5,10); //  Àü±â 
					}
                }
                if ( o->SubType==10 )
                {
				...
				)
            {
                if ( o->Weapon<=0 ) //  µô·¹ÀÌ.
                {
                    ...
                    for ( int i=1; i<o->MultiUse; ++i )
                    {
						...
						else if(o->SubType >= 11 || o->SubType <= 13)	//^ Ææ¸± ÀÌÆåÆ® °ü·Ã
						{
							o->TargetPosition[1] -= 20.f;
						}
                        else
                ...
                if ( o->m_bCreateTails && o->PKKey!=1 )
                {
                    if ( o->SubType<2 )
                    {
                        ...
                        if ( o->Target!=NULL )
                        {
                            ...
                            if ( o->SubType==0 )        //  ¼®È­ ¸¶¹ý Á¦°Å.
                            {
                                b->RenderMeshEffect ( 0, MODEL_SKIN_SHELL );
                            }
                            else if ( o->SubType==1 )   //  Åõ¸í ¸¶¹ý Á¦°Å.
                            {
...
void RenderJoints( BYTE bRenderOneMore )
{
	...
	for ( int i=0; i<MAX_JOINTS; i++ )
	{
		...
		if ( o->Live && o->NumTails>0 && o->RenderFace!=0 )
		{
            ...
            if ( o->Type == MODEL_SPEARSKILL)	// ¹æ¾î¸·
			{
                float fAlpha;
				switch ( o->SubType)
				{
