...
void CNewUIStorageInventory::ProcessStorageItemAutoMove2()
{
	// 커서에 아이템이 붙어 있으면 처리하지 않음.
	if (g_pPickedItem)
		if (g_pPickedItem->GetItem())
	...
	CNewUIInventoryCtrl* pMixInvenCtrl = g_pMixInventory->GetInventoryCtrl();
	ITEM* pItemObj = pMixInvenCtrl->FindItemAtPt(MouseX, MouseY);

	//ITEM* pItemObj = m_pNewInventoryCtrl->FindItemAtPt(MouseX, MouseY);
	g_pChatListBox->AddText("", "ProcessStorageItemAutoMove2 1", SEASON3B::TYPE_SYSTEM_MESSAGE);
	if (pItemObj)
	{
		g_pChatListBox->AddText("", "ProcessStorageItemAutoMove2 pItemObj", SEASON3B::TYPE_SYSTEM_MESSAGE);
		int nDstIndex = g_pMyInventory->FindEmptySlot(pItemObj);
		if (-1 != nDstIndex)
		{
			g_pChatListBox->AddText("", "ProcessStorageItemAutoMove2 nDstIndex", SEASON3B::TYPE_SYSTEM_MESSAGE);
			g_pMixInventory->SetItemAutoMove(true);
			
			int nSrcIndex
				= pItemObj->y * pMixInvenCtrl->GetNumberOfColumn()
					+ pItemObj->x;
			//SendRequestItemToMyInven(pItemObj, nSrcIndex, MAX_EQUIPMENT_INDEX+nDstIndex);
			//(ITEM* pItemObj, int nStorageIndex, int nInvenIndex)

			SendRequestEquipmentItem(REQUEST_EQUIPMENT_CHAOS_MIX, nSrcIndex,
			pItemObj, REQUEST_EQUIPMENT_INVENTORY, MAX_EQUIPMENT_INDEX+nDstIndex);
...
void CNewUIStorageInventory::ProcessMyInvenItemAutoMove2()
{
	if (g_pPickedItem)
		if (g_pPickedItem->GetItem())
			return;

	g_pChatListBox->AddText("", "ProcessMyInvenItemAutoMove2 1", SEASON3B::TYPE_SYSTEM_MESSAGE);

	// 아이템 자동 이동 중일 때 또 아이템 자동 이동인 경우는 막음.
	if (IsItemAutoMove())
		return;

	g_pChatListBox->AddText("", "ProcessMyInvenItemAutoMove2 2", SEASON3B::TYPE_SYSTEM_MESSAGE);

	CNewUIInventoryCtrl* pMyInvenCtrl = g_pMyInventory->GetInventoryCtrl();
	ITEM* pItemObj = pMyInvenCtrl->FindItemAtPt(MouseX, MouseY);
	if (pItemObj)
	{
		g_pChatListBox->AddText("", "ProcessMyInvenItemAutoMove2 3", SEASON3B::TYPE_SYSTEM_MESSAGE);
		if (pItemObj->Type == ITEM_HELPER+20)	// 마법사의 반지, 용사의 반지, 전사의 반지
			return;

		int nDstIndex = g_pMixInventory->FindEmptySlot(pItemObj);
		if (-1 != nDstIndex)
		{
			g_pChatListBox->AddText("", "ProcessMyInvenItemAutoMove2 4", SEASON3B::TYPE_SYSTEM_MESSAGE);
			SetItemAutoMove(true);
			...
			//(ITEM* pItemObj, int nInvenIndex, int nStorageIndex)
			SendRequestEquipmentItem(REQUEST_EQUIPMENT_INVENTORY, MAX_EQUIPMENT_INDEX+nSrcIndex,
			pItemObj, REQUEST_EQUIPMENT_CHAOS_MIX, nDstIndex);
...
void CNewUIStorageInventory::ProcessToReceiveStorageItems(int nIndex, BYTE* pbyItemPacket)
{
		...
		* m_pNewInventoryCtrl->GetNumberOfRow()))
	{
		if (IsItemAutoMove())
		{
			g_pChatListBox->AddText("", "ProcessToReceiveStorageItems IsItemAutoMove OK", SEASON3B::TYPE_SYSTEM_MESSAGE);
			CNewUIInventoryCtrl* pMyInvenCtrl = g_pMyInventory->GetInventoryCtrl();
