...
int CutStr(const char* pszSrcText, char * pTextOut, const int iTargetPixelWidth, const int iMaxOutLine, const int iOutStrLength, const int iFirstLineTab /* = 0 */)
{
	...
	int iScreenRatePixelWidth = iTargetPixelWidth*g_fScreenRate_x-5;				// Å¸°Ù ÇÈ¼¿ ±æÀÌ¸¦ ½ºÅ©¸°¿¡ µû¶ó À¯µ¿ÀûÀ¸·Î ¹Ù²Û´Ù.(-5Àº ¿©¹éÀ» ³²±â±â À§ÇÔ)
	SIZE iSize;
	...
	while (iCharIndex < wstrUTF16.length() && iLineIndex < iMaxOutLine)
	{
		...
		// Ã¹¹øÂ° ÁÙÀº ³Ñ¾î¿Â °ª(¿¹: ÀÌ¸§ Ç¥½Ã ±æÀÌ)¸¸Å­À» »« ³ª¸ÓÁö·Î °è»êÇÑ´Ù.
		if (iLineIndex == 0) 
			iSize.cx += iFirstLineTab;

		// 1. ÇöÀç ±ÛÀÚ¸¦ Æ÷ÇÔÇßÀ» ¶§ÀÇ ¹®ÀÚ¿­ÀÇ ±æÀÌ°¡ Á¦ÇÑµÈ ±æÀÌº¸´Ù Å©°Å³ª 
		// 2. Array 'pTextOut'ÀÇ size º¸´Ù Å¬ °æ¿ì  
		if (iSize.cx >= iScreenRatePixelWidth || strText.length() >= iOutStrLength-1)
		{
			// ºóÄ­ÀÌ ÀÖÀ» °æ¿ì ¹Ù·Î Àü ºóÄ­±îÁö 
			// ºóÄ­ÀÌ ¾øÀ» °æ¿ì ¹Ù·Î ÀüÀÇ ±ÛÀÚ±îÁö UTF-8·Î º¯È¯ÇÑ ÈÄ ÀúÀå
			...
			// º¹»çµÉ array¸¦ °¡¸®Å°°í ÀÖ´Â pTextOutÆ÷ÀÎÅÍÀÇ À§Ä¡¸¦ iOutStrLength¸¸Å­ µÚ·Î ¿Å±ä´Ù.
			// (pTextOut[i][] => pTextOut[i+1][] °ú °°Àº ¿ªÇÒ)
			iLineIndex++;
	...
	//memset(pTextOut, 0, iMaxOutLine * iOutStrLength);		// ¸Þ¸ð¸® Ä§¹üÇÔ.
	
	const char *pWorkStr = pszSrcText;						// ÀÛ¾÷¿ë Str Æ÷ÀÎÅÍ
	int iSrcTotalStrLength = unicode::_strlen(pszSrcText)+1;				// ÀüÃ¼ ¹®ÀÚ°¹¼ö(Null¹®ÀÚ Æ÷ÇÔ)
	int iDestStrLineIndex = 0;									// ÇöÀç ¶óÀÎ
	int iDestTotalStrLength = 0;								// ÇöÀç ¹®ÀÚ °¹¼ö
	bool bEndofSrcStr = false;									// ¿øº» ¹®ÀÚ¿­ÀÇ ³¡ÀÎ°¡?
	int iStrByteSize = 0;					

	int iScreenRatePixelWidth = iTargetPixelWidth*g_fScreenRate_x;				// Å¸°Ù ÇÈ¼¿ ±æÀÌ¸¦ ½ºÅ©¸°¿¡ µû¶ó À¯µ¿ÀûÀ¸·Î ¹Ù²Û´Ù.
	...
	for( iDestStrLineIndex = 0 ; iDestStrLineIndex < iMaxOutLine ; iDestStrLineIndex++ )
	{
		...
		{
			iStrByteSize = _mbclen((unsigned char*)&pWorkStr[iDestStrLength]);		// ÇÑ ¹®ÀÚ´ç ¹ÙÀÌÆ®
...
// Wrapped FunctionÀÓ, ÀÌ°É ¾²Áö¸»°í, CutStrÀ» »ç¿ëÇÏ»ï.
...
int CutText3(const char* pszText, char * pTextOut, const int TargetWidth, const int iMaxOutLine, const int iOutStrLength, const int iFirstLineTab, const BOOL bReverseWrite)
{
	...
	while(1)
	{
		...
		char * pszTextOut = pTextOut + iOutStrLength * (bReverseWrite == TRUE ? iMaxOutLine - 1 - index : index);	// °á°ú ÀúÀå¿ë
...
#endif //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
		int TextWidth = int(TextSize.cx / g_fScreenRate_x);	// ÀÛ¾÷ ¹®Àå ³Êºñ
		...
		if (TextWidth > TargetWidth - (index == 0 ? iFirstLineTab : 0))
		{
			...
			// ±æÀÌ Á¤ÇÏ±â
			int resultLength = (TargetWidth - (index == 0 ? iFirstLineTab : 0)) / (TextWidth / lstrlen(pWorkText));	// ±æÀÌ·Î ¸ÂÃçº¸°í ½ÃÀÛ
			...
			while(TRUE)
			{
				...
				else
				{
					...
					// ¹öÆÛ¸¦ ÀÛ°Ô Àâ¾ÒÀ»°æ¿ì ¹öÆÛ¸¦ ¿ì¼±À¸·Î ÀÚ¸¥´Ù
			...
			for (int i = 0; i < resultLength;)
			{
				...
				i += _mbclen(( unsigned char*)&pWorkText[i]);	// ÇÑ±ÛÀÌ¸é 2, ¾Æ´Ï¸é 1 À» ¸®ÅÏ
				iTextSize = i;
			}
...
DWORD g_dwActiveUIID = 0;	// ÇöÀç ÀÛµ¿ÁßÀÎ UI ID
DWORD g_dwMouseUseUIID = 0;		// ¸¶¿ì½º°¡ UI À§¿¡ ÀÖ´Â°¡ (ÀÖÀ¸¸é UI ID)

DWORD g_dwTopWindow = 0;
DWORD g_dwKeyFocusUIID = 0;	// Å°º¸µå ¹ÝÀÀÀ» ¹ÞÀ» UI ID
DWORD g_dwCurrentPressedButtonID = 0;	// ÇöÀç ´­¸°¹öÆ°
////////////////////////////////////////////////////////////////////////////////////////////////////
...
BOOL CUIControl::DoAction(BOOL bMessageOnly)
{
	...
	{
		GetUIMessage();		// ¸Þ½ÃÁö °¡Á®¿À±â
		if (HandleMessage() == FALSE)	// ¸Þ½ÃÁö Ã³¸®
	...
	case UI_MESSAGE_P_RESIZE:
		if (m_dwParentUIID != 0)
		{
			...
			if (pWindow != NULL)
			{
				...
				{
				case 0:	// Àý´ë;Àý´ë
					if (m_iRelativeWidth == 0 && m_iRelativeHeight == 0) break;	// »çÀÌÁî º¯ÇÏÁö ¾Ê´Â°Í
...
{
	if (GetState() == UISTATE_DISABLE)	// »ç¿ë ¸øÇÏ´Â ¹öÆ°
	{
		return FALSE;
	}
...
{
	// Æ² Ãâ·Â
	RenderInterface();	// ´Ù¸¥ ¸®½ºÆ® Å¬·¡½ºµéÀÌ »ç¿ëÇÏ°í ÀÖÀ¸´Ï ¸·Áö¸¶½Ã°í »ó¼Ó¹ÞÀº ÇÔ¼ö¿¡¼­ return ÇØÁÖ¼¼¿ä!!!
	// ³»¿ë
	MoveRenderLine();
	...
	for (int i = 0; i < m_iNumRenderLine; ++i)
	{
		...
		if (bResult < 0)	// °ø¶õ »ðÀÔ (-1 * °ø¶õ °³¼ö)
		{
			i -= bResult;
		}
		else if (bResult == FALSE)	// ¶óÀÎ ½ºÅµ
		{
			--i;
		}
		++m_TextListIter;
...
void CUITextListBox<T>::ComputeScrollBar()
{
	if (m_bUseNewUIScrollBar == TRUE)
	{
		...
		m_fScrollBarHeight = 20;	// Å©±â Á¦ÇÑ
	...
	else
	{
		...
		if (m_fScrollBarHeight < 2) m_fScrollBarHeight = 2;	// Å©±â Á¦ÇÑ
...
BOOL CUITextListBox<T>::HandleMessage()
{
	...
	case UI_MESSAGE_P_RESIZE:
		...
		break;
	case UI_MESSAGE_LISTSCRLTOP:	// ½ºÅ©·Ñ ¸Ç À§·Î
		Scrolling(-1000);
	...
	case UI_MESSAGE_LISTSELDOWN:
		if (m_bUseSelectLine == TRUE && m_TextList.size() >= 1)
		{
			...
			int iTarget = 0;
			if (iLine >= m_iCurrentRenderEndLine + m_iNumRenderLine)	// ½ºÅ©·Ñ À§¿¡
			{
				iTarget = iLine - m_iNumRenderLine + 1;
			}
			else if (iLine < m_iCurrentRenderEndLine)	// ½ºÅ©·Ñ ¾Æ·¡¿¡
			{
				iTarget = iLine;
			}
			else break;
...
CUIGuildListBox::CUIGuildListBox()
{
	...
	m_iCurrentRenderEndLine = 0;
	m_iNumRenderLine = 24;	// 3ÀÇ ¹è¼ö·Î -_-
...
//	SetPosiion(213, 415);
//	SetSize(213, 250);
	m_iNumRenderLine = 18;	// 3ÀÇ ¹è¼ö·Î -_-
	SetPosition(460, 340);
	SetSize(170, 250);
}
////////////////////////////////////////////////////////////////////////////////////////////////////v
...
void CUIGuildListBox::RenderInterface()
{
	...
	if (GetLineNum() >= m_iNumRenderLine)
	{
		...
		RenderBitmap(BITMAP_INTERFACE_EX+4, (float)m_iPos_x + m_iWidth - 19, (float)m_iPos_y - m_iHeight + 8,
			13.0f, 13.0f, 0.0f, 0.0f, 13.0f/16.0f, 13.0f/16.0f);	// ¡ã
		EnableAlphaTest();
		RenderBitmap(BITMAP_INTERFACE_EX+4, (float)m_iPos_x + m_iWidth - 19, (float)m_iPos_y - 4,
			13.0f, -13.0f, 0.0f, 0.0f, 13.0f/16.0f, 13.0f/16.0f);	// ¡å
		DisableAlphaBlend();
...
BOOL CUIGuildListBox::RenderDataLine(int iLineNumber)
{
	...
	if(++m_TextListIter == m_TextList.end())	// ±æµå ¸¶½ºÅÍ
	{
		--m_TextListIter;
...
{
	if (m_dwParentUIID != 0)	// »çÀÌÁî ¹Ù²Ù°í ½ÃÀÛÇÏÀÚ
	{
		CUIBaseWindow * pWindow = g_pWindowMgr->GetWindow(m_dwParentUIID);
		if (pWindow != NULL)
		{
...
void CUISimpleChatListBox::AddTextToRenderList(const char * pszID, const char * pszText, int iType, int iColor)
{
	...
	if (strlen(pszID) + strlen(pszText) >= 20)	// »çÀÌÁî ¿À¹ö
	{
		SIZE nameSize;
#ifdef LJH_ADD_SUPPORTING_MULTI_LANGUAGE
...
#ifdef LJH_ADD_SUPPORTING_MULTI_LANGUAGE
	g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), GlobalText[1022], lstrlen(GlobalText[1022]), &TextSize);	// " ¼­¹öÀ§Ä¡ "
#else  //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
	unicode::_GetTextExtentPoint(g_pRenderText->GetFontDC(), GlobalText[1022], lstrlen(GlobalText[1022]), &TextSize);	// " ¼­¹öÀ§Ä¡ "
#endif //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
...
BOOL CUIChatPalListBox::RenderDataLine(int iLineNumber)
{
	...
	if (m_iLayoutType == 1)
	{
		...
		{
			sprintf(Text,GlobalText[1039]);	// "¿ÀÇÁ¶óÀÎ"
		}
		else if (m_TextListIter->m_Server == 0xFE)
		{
			sprintf(Text,GlobalText[1039]);	// "´ëÈ­°ÅºÎ"6
		}
		else if (m_TextListIter->m_Server == 0xFD)
		{
			sprintf(Text,GlobalText[1041]);	// "»ç¿ëºÒ°¡"
		}
...
//		{
//			sprintf(Text,GlobalText[1040]);	// "ÀÀ´ä´ë±â"
//		}
		else
		{	// Á¢¼ÓÁßÀÏ¶§´Â ¼­¹ö ÀÌ¸§ Ç¥½Ã
			sprintf(Text, GlobalText[1042], m_TextListIter->m_Server + 1);	// " %2d ¼­¹ö"
		}
...
void RenderCheckBox(int iPos_x, int iPos_y, BOOL bFlag)
{
	...
	EnableAlphaTest();
	if (bFlag == TRUE)
		RenderBitmap(BITMAP_INTERFACE_EX+13,iPos_x + 2,iPos_y + 2,5.0f,5.0f,0.f,0.f,5.f/8.f,5.f/8.f);	// vÃ¼Å©
}
...
#ifdef LJH_ADD_SUPPORTING_MULTI_LANGUAGE
	g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), GlobalText[1028], lstrlen(GlobalText[1028]), &TextSize);	// " º¸³½»ç¶÷ "
#else  //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
	unicode::_GetTextExtentPoint(g_pRenderText->GetFontDC(), GlobalText[1028], lstrlen(GlobalText[1028]), &TextSize);	// " º¸³½»ç¶÷ "
#endif //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
	SetColumnWidth(1, TextSize.cx / g_fScreenRate_x + 8);
#ifdef LJH_ADD_SUPPORTING_MULTI_LANGUAGE
	g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), GlobalText[1029], lstrlen(GlobalText[1029]), &TextSize);	// " ¹ÞÀº³¯Â¥ "
#else  //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
	unicode::_GetTextExtentPoint(g_pRenderText->GetFontDC(), GlobalText[1029], lstrlen(GlobalText[1029]), &TextSize);	// " ¹ÞÀº³¯Â¥ "
#endif //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
	SetColumnWidth(2, TextSize.cx / g_fScreenRate_x + 8);
#ifdef LJH_ADD_SUPPORTING_MULTI_LANGUAGE
	g_pMultiLanguage->_GetTextExtentPoint32(g_pRenderText->GetFontDC(), GlobalText[1030], lstrlen(GlobalText[1030]), &TextSize);	// " Á¦  ¸ñ "
#else  //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
	unicode::_GetTextExtentPoint(g_pRenderText->GetFontDC(), GlobalText[1030], lstrlen(GlobalText[1030]), &TextSize);	// " Á¦  ¸ñ "
#endif //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
...
BOOL CUILetterListBox::RenderDataLine(int iLineNumber)
{
	...
	if (m_TextListIter->m_bIsRead == TRUE)
		RenderBitmap(BITMAP_INTERFACE_EX+14,iPos_x + 1 + 10,iPos_y - 2,13.0f,10.0f,0.f,9.f/32.f,13.f/16.f,10.f/32.f);	// vÃ¼Å©
	else
		RenderBitmap(BITMAP_INTERFACE_EX+14,iPos_x + 1 + 10,iPos_y - 1,13.0f,9.0f,0.f,0.f,13.f/16.f,9.f/32.f);	// vÃ¼Å©
...
void CUILetterTextListBox::AddTextToRenderList(const char * pszText)
{
	...
	static LETTER_TEXT text;
	if (strlen(pszText) >= 20)	// »çÀÌÁî ¿À¹ö
	{
...
{
	if(iRenderTextType == 0)	// ±¸¹öÀü
	{
		m_pRenderText = new CUIRenderTextOriginal;
		if(false == m_pRenderText->Create(hDC))
			return false;
	}
	else if(iRenderTextType == 1)	// »õ¹öÀü
	{
		//m_pRenderText = new CUIRenderTextAdvance;		/* »ç¿ëÇÏÁö ¾ÊÀ¸¹Ç·Î */
	}
	return true;
}
...
bool CUIRenderTextOriginal::Create(HDC hDC)
{
    ...
    DIB_INFO->bmiHeader.biWidth       = 640*g_fScreenRate_x;		//. 640 ±âÁØ
	DIB_INFO->bmiHeader.biHeight      = -(480*g_fScreenRate_y);		//. 480 ±âÁØ
...
{
	const int LIMIT_WIDTH = 256, LIMIT_HEIGHT = 32;		// ÆùÆ® ÅØ½ºÃÄÀÇ °¡·Î, ¼¼·Î »çÀÌÁî
	...
	for(int y = 0; y < iHeight; ++y)
	{
		...
		for(int x = 0; x < iWidth; ++x)
		{
			...
			{
#ifdef _DEBUG
				__asm { int 3 };			//. Àý´ë·Î µé¾î¿À¸é ¾ÈµÊ: ²À µð¹ö±ë ÇÏ¼¼¿ä.
#endif // _DEBUG
				return;
			}
			...
			else									// ¹è°æ
			{
				...
				*((unsigned int *)(pBitmapFont->Buffer + DstIndex)) = 0;	//. Åõ°ú
			}
			SrcIndex += 3;
			DstIndex += 4;
		}
...
// ÇÔ¼ö ÀÌ¸§ : RenderText()
// ÇÔ¼ö ¼³¸í : ÅØ½ºÆ® ·»´õ.
// ¸Å°³ º¯¼ö : iPos_x		: ·»´õµÉ ¿µ¿ªÀÇ xÁÂÇ¥.
//			   iPos_y		: ·»´õµÉ ¿µ¿ªÀÇ yÁÂÇ¥.
//			   pszText		: ·»´õÇÒ ¹®ÀÚ¿­.
//			   iBoxWidth	: ·»´õµÉ ¿µ¿­ÀÇ ³Êºñ.
//			   iBoxHeight	: ·»´õµÉ ¿µ¿­ÀÇ ³ôºñ.
//			   iSort		: ·»´õµÉ ¿µ¿ª¿¡¼­ÀÇ Á¤·Ä ±âÁØ.
//							 (RT3_SORT_LEFT, RT3_SORT_CENTER, RT3_SORT_RIGHT,
//							 RT3_WRITE_RIGHT_TO_LEFT, RT3_SORT_LEFT_CLIP,
//							 RT3_SORT_LEFT_IGNORE Áß ÇÏ³ª)
//			   iBorder		: »ç¿ëÇÏÁö ¾ÊÀ½.
//			   iClipWidth	: »ç¿ëÇÏÁö ¾ÊÀ½.
//*****************************************************************************
										...
										int iSort /* = RT3_SORT_LEFT */, OUT SIZE* lpTextSize /* = NULL */)
{
	...
	SIZEF RealRenderingSize = { RealTextSize.cx, RealTextSize.cy };		// ÇöÀç ÇØ»óµµ ±âÁØ
	...
	int iTab = 0;
	int iClipMove = 0;
	// ¿ÞÂÊ Å¬¸®ÇÎ Á¤·Ä. (¹Ú½º ±âÁØ)
	if (iSort == RT3_SORT_LEFT_CLIP)
	{
		if(RealRenderingSize.cx > RealBoxSize.cx)		//. ·»´õ¿µ¿ªÀÌ ¹Ú½ºÅ©±â º¸´Ù Å« °æ¿ì
		{
			iClipMove = RealRenderingSize.cx - RealBoxSize.cx;	// ¿ÞÂÊÀ¸·Î Àß¸° ¿µ¿ª ³Êºñ.
	...
	else if(iSort == RT3_SORT_CENTER)
	{
		...
		else											//. ·»´õ¿µ¿ªÀÌ ¹Ú½ºÅ©±â º¸´Ù ÀÛÀº °æ¿ì
		{
	...
	if(pszText[0] != 0x0a)	// ¹®ÀÚ¿­ Ã¹¹øÂ° Ä³¸¯ÅÍ°¡ ´ÙÀ½ ¶óÀÎ(LF)ÀÌ ¾Æ´Ï¸é.
	{
	...
	//. 256(ÅØ½ºÃÄ »çÀÌÁî)À» ³Ñ¾î°¡´Â ÅØ½ºÆ®´Â x°³ÀÇ ±¸°£À¸·Î ÂÉ±ü´Ù.
	//. 256À» ³ÑÁö ¾ÊÀ¸¸é iNumberOfSections == 1ÀÌ µÈ´Ù
	...
	if(lpTextSize)		//. Ãâ·ÂµÈ ÅØ½ºÆ® »çÀÌÁî¸¦ ³Ñ°ÜÁØ´Ù. (640*480 ±âÁØ)
	{
	...
	if (iMode & IME_CONVERSIONMODE)
	{
		...
		else
		{
			...
			if (dwConv != IME_CMODE_ALPHANUMERIC)
			{
				...
				// IME »óÅÂ¸¦ ¿µ¹®À¸·Î
				//CreateWhisper("","IMEConv »óÅÂ¸¦ ¿µ¹®À¸·Î",3);
	...
	if (iMode & IME_SENTENCEMODE)
	{
		...
		else
		{
			...
			if (dwSent != IME_SMODE_NONE)
			{
				...
				// IME »óÅÂ¸¦ ¿µ¹®À¸·Î
				//CreateWhisper("","IMESent »óÅÂ¸¦ ¿µ¹®À¸·Î",3);
...
#endif // KWAK_FIX_KEY_STATE_RUNTIME_ERR
	{
		pTextInputBox->m_iCaretBlinkTemp = 0;	// ¹æÇâÅ° ´­·¶À»¶§ Ä¿¼­ ±ô¹ÚÀÓ Ã³¸®
	}
	...
	case WM_IME_NOTIFY:
		...
		if (msg == WM_IME_NOTIFY && (pTextInputBox->CheckOption(UIOPTION_SERIALNUMBER) || pTextInputBox->CheckOption(UIOPTION_NUMBERONLY)))
		{
			...
			case IMN_SETOPENSTATUS:
				//if (SELECTED_LANGUAGE == LANGUAGE_JAPANESE)
				{
					// Èçìåíèòü CONVERSIONMODE áåç ðàçãîâîðà (ßïîíèÿ)
...
void CUITextInputBox::SetIMEPosition()
{
	...
	int iSetPos_x = m_iPos_x * g_fScreenRate_x + WindowWidth;	// ¼³Á¤µÈ x,y
...
void CUITextInputBox::GiveFocus(BOOL bSel)
{
	...
	if (bSel == TRUE)
#ifdef LJH_ADD_SUPPORTING_MULTI_LANGUAGE 
		PostMessageW(m_hEditWnd, EM_SETSEL, ( WPARAM)0, ( LPARAM)-1);	// ÀüÃ¼ ¼±ÅÃ
#else //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
		PostMessage(m_hEditWnd, EM_SETSEL, ( WPARAM)0, ( LPARAM)-1);	// ÀüÃ¼ ¼±ÅÃ
#endif //LJH_ADD_SUPPORTING_MULTI_LANGUAGE	
	else
#ifdef LJH_ADD_SUPPORTING_MULTI_LANGUAGE 
		PostMessageW(m_hEditWnd, EM_SETSEL, ( WPARAM)-2, ( LPARAM)-1);	// Ä¿¼­ ¸Ç µÚ·Î
#else //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
		PostMessage(m_hEditWnd, EM_SETSEL, ( WPARAM)-2, ( LPARAM)-1);	// Ä¿¼­ ¸Ç µÚ·Î
#endif //LJH_ADD_SUPPORTING_MULTI_LANGUAGE	
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CUITextInputBox::WriteText(int iOffset, int iWidth, int iHeight)
{
	...
	for(int y = 0; y < iHeight; ++y)
	{
		...
		for(int x = 0; x < iWidth; ++x)
		{
			...
			if(*(m_pFontBuffer+SrcIndex) == 255)	// ±ÛÀÚ
			{
				//. ÇöÀç ÇÈ¼¿ÀÌ Ä³·µ ¿µ¿ª¾È¿¡ ÀÖ´Â°¡?
				if(bIsCaretTime && PtInRect(&rcCaret, ptProcessing))
					*((unsigned int *)(pBitmapFont->Buffer + DstIndex)) = m_dwBackColor;
				else
					*((unsigned int *)(pBitmapFont->Buffer + DstIndex)) = m_dwTextColor;
			}
			else if(*(m_pFontBuffer+SrcIndex) == 0)	// ¹è°æ
			{
				//. ÇöÀç ÇÈ¼¿ÀÌ Ä³·µ ¿µ¿ª¾È¿¡ ÀÖ´Â°¡?
				if(bIsCaretTime && PtInRect(&rcCaret, ptProcessing))
...
void CUITextInputBox::Render()
{
...
#endif //LJH_ADD_SUPPORTING_MULTI_LANGUAGE
		m_fCaretHeight = TextSize.cy;		// ÇöÀç ÇØ»óµµ ±âÁØÀ¸·Î »ç¿ë
	}
#if SELECTED_LANGUAGE == LANGUAGE_KOREAN
	...
	const int LIMIT_WIDTH = 256, LIMIT_HEIGHT = 32;		// ÆùÆ® ÅØ½ºÃÄÀÇ °¡·Î, ¼¼·Î »çÀÌÁî
	SIZE RealTextLine = { 0, 0 };		// ÇöÀç ÇØ»óµµ ±âÁØ (Ä³·µÆ÷ÇÔ)
	
	if (m_bUseMultiLine == FALSE)	//. ½Ì±Û ¶óÀÎ
	{
		...
		if(RealTextLine.cx > RealWndSize.cx)		//. ·»´õ¿µ¿ª ³Êºñº¸´Ù ¹®ÀÚ¿­ ³Êºñ°¡ Å©´Ù¸é
			RealTextLine.cx = RealWndSize.cx;
		if(RealTextLine.cy > RealWndSize.cy)		//. ·»´õ¿µ¿ª ³Êºñº¸´Ù ¹®ÀÚ¿­ ³ôÀÌ°¡ Å©´Ù¸é
	...
	else							//. ¸ÖÆ¼ ¶óÀÎ
	{
		...
		if(GetUseScrollbar())
#endif //PBG_ADD_INGAMESHOPMSGBOX
		RenderScrollbar();		// ½ºÅ©·Ñ¹Ù
	}

	++m_iCaretBlinkTemp;
	
#if SELECTED_LANGUAGE == LANGUAGE_TAIWANESE
	if (HaveFocus() == TRUE)	// Á¶ÇÕÁßÀÎ ÅØ½ºÆ® Ç¥½Ã
	{
		POINT pt;
		GetCaretPos(&pt);
...
{
	float fScrollPos = GetScrollPos(m_hEditWnd, SB_VERT);	// ½ºÅ©·Ñ À§Ä¡ (0~
	float fLineNum = SendMessage(m_hEditWnd, EM_GETLINECOUNT, 0, 0);	// ¶óÀÎ¼ö (1~
	//m_iNumLines = 15;	// ÇÑÈ­¸é¿¡ ³ª¿À´Â ¶óÀÎ¼ö
...
void CUIChatInputBox::TabMove(int iBoxNumber)
{
	...
	if (iBoxNumber == 1)// && GetFocus() == m_TextInputBox.GetHandle())	// ±Ó¼Ó¸»
	{
		m_BuddyInputBox.GiveFocus();
		PostMessage(m_BuddyInputBox.GetHandle(), EM_SETSEL, ( WPARAM)0, ( LPARAM)-1);
	}
	else// if (GetFocus() == m_BuddyInputBox.GetHandle())	// ±âº»
	{
		m_TextInputBox.GiveFocus();
...
void CUISlideHelp::AddSlide(int iLoopCount, int iLoopDelay, const char * pszText, int iType, float fSpeed, DWORD dwTextColor)
{
	...
	// °øÁö Type
	//	1	- ±ä±Þ ¸Þ½ÃÁö (Áß¿äµµ »ó) : ´Ù¸¥ °øÁö ¹«½ÃÇÏ°í ¹Ù·Î ´ÙÀ½¹ø¿¡ µîÀå
	//	0	- ÇÊµ¶ ¸Þ½ÃÁö (Áß¿äµµ Áß) : °øÁö ½Ã°£¿¡ ¸Â°Ô µîÀå
	//	-1	- ÀÏ¹Ý ¸Þ½ÃÁö (Áß¿äµµ ÇÏ) : °øÁö ½Ã°£¿¡ ¸Â°Ô µîÀåÇÏ³ª µîÀå ½Ã°£ÀÌ ÁöÃ¼µÉ °æ¿ì »èÁ¦µÊ
...
void CUISlideHelp::ManageSlide()
{
	...
	for (m_SlideQueueIter = m_SlideQueue.begin(); m_SlideQueueIter != m_SlideQueue.end(); ++m_SlideQueueIter)
	{
		...
		else
		{
			...
			BOOL bResult = FALSE;
#endif // KWAK_FIX_COMPILE_LEVEL4_WARNING
			if ( m_SlideQueueIter->second.m_iType == -1 && m_SlideQueueIter->first + 60 < m_dwCurrentSecond );	// ½Ã°£ Áö³ª¸é »èÁ¦
...
void CSlideHelpMgr::OpenSlideTextFile(const char * szFileName)	// slide.bmd ·Îµù
{
...
#endif // _VS2008PORTING
	{
		...
		{
			//if (m_SlideTextList[i].size() >= m_iTextNumber[i]) break;	// ´õÀÌ»ó ¸ø³Ö°Ô
...
void CSlideHelpMgr::ClearSlideText()						// ½½¶óÀÌµå ÅØ½ºÆ® Áö¿ì±â
{
	for (int i = 0; i < SLIDE_LEVEL_MAX; ++i)
	{
		m_iLevelCap[i] = 0;
...
const char * CSlideHelpMgr::GetSlideText(int iLevel)			// ÇöÀç ·¹º§ÀÇ ½½¶óÀÌµå ÅØ½ºÆ® ¾ò±â
{
	int iHelpType = -1;
	for (int i = 0; i < SLIDE_LEVEL_MAX; ++i)
	{
	...
	if (iHelpType == -1) return NULL;	// ½½¶óÀÌµå Ç¥½Ã ·¹º§ ÃÊ°ú
...
void CSlideHelpMgr::AddSlide(int iLoopCount, int iLoopDelay, const char * pszText, int iType, float fSpeed, DWORD dwTextColor)
{
	...
	// °øÁö Type
	//	5	- ±ä±Þ °øÁö (Áß¿äµµ ÃÖ»ó)	: ´Ù¸¥ °øÁö ¹«½ÃÇÏ°í ¹Ù·Î ´ÙÀ½¹ø¿¡ µîÀå
	//	4	- ÇÊµ¶ °øÁö (Áß¿äµµ »ó)		: °øÁö ½Ã°£¿¡ ¸Â°Ô µîÀå
	//	3	- ÀÏ¹Ý °øÁö (Áß¿äµµ Áß»ó)	: °øÁö ½Ã°£¿¡ ¸Â°Ô µîÀåÇÏ³ª µîÀå ½Ã°£ÀÌ ÁöÃ¼µÉ °æ¿ì »èÁ¦µÊ
	//	2	- ±ä±Þ ¸Þ½ÃÁö (Áß¿äµµ Áß)	: ´Ù¸¥ °øÁö ¹«½ÃÇÏ°í ¹Ù·Î ´ÙÀ½¹ø¿¡ µîÀå
	//	1	- ÇÊµ¶ ¸Þ½ÃÁö (Áß¿äµµ ÇÏ)	: °øÁö ½Ã°£¿¡ ¸Â°Ô µîÀå
	//	0	- ÀÏ¹Ý ¸Þ½ÃÁö (Áß¿äµµ ÃÖÇÏ)	: °øÁö ½Ã°£¿¡ ¸Â°Ô µîÀåÇÏ³ª µîÀå ½Ã°£ÀÌ ÁöÃ¼µÉ °æ¿ì »èÁ¦µÊ
	switch(iType)
	{
...
BOOL CUINewGuildMemberListBox::RenderDataLine(int iLineNumber)
{
	...
	int iCharacterLevel;
	if (m_TextListIter->m_GuildStatus == 128)	// ±æµå¸¶½ºÅÍ
		iCharacterLevel = 0;
	else if (m_TextListIter->m_GuildStatus == 64)	// ºÎ±æµå¸¶½ºÅÍ
		iCharacterLevel = 1;
	else if (m_TextListIter->m_GuildStatus == 32)	// ¹èÆ²¸¶½ºÅÍ
		iCharacterLevel = 2;
	else	// ÀÏ¹Ý ±æµå¿ø
		iCharacterLevel = 3;

	if (iCharacterLevel == 0)	// ±æµå¸¶½ºÅÍ
	{
		glColor4ub(255, 100, 50, 127);
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		glColor4f(255, 255, 255, 255);
	}
	else if (iCharacterLevel == 1)	// ºÎ±æµå¸¶½ºÅÍ
	{
		glColor4ub(255, 150, 80, 127);
		RenderColor(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13);
		glColor4f(255, 255, 255, 255);
	}
	else if (iCharacterLevel == 2)	// ¹èÆ²¸¶½ºÅÍ
	{
...
//	else g_pRenderText->RenderText(iPos_x + 45, iPos_y, "ÀÏ¹Ý ±æµå¿ø", 60, 0, RT3_SORT_CENTER);
...
#ifdef LEM_ADD_SEASON5_PART5_MINIUPDATE_JEWELMIX // º¸¼®Á¶ÇÕ ÇØÃ¼ ¸®½ºÆ® Ãâ·Â
...
void CUIBCDeclareGuildListBox::RenderInterface()
{
	...
	g_pRenderText->RenderText(m_iPos_x+50, m_iPos_y - m_iHeight - 12, GlobalText[1528] );	// 1528 "µî·Ï¼ö"
#else // SELECTED_LANGUAGE == LANGUAGE_ENGLISH || SELECTED_LANGUAGE == LANGUAGE_PHILIPPINES || SELECTED_LANGUAGE == LANGUAGE_VIETNAMESE
	g_pRenderText->RenderText(m_iPos_x+63, m_iPos_y - m_iHeight - 12, GlobalText[1528] );	// 1528 "µî·Ï¼ö"
#endif // SELECTED_LANGUAGE == LANGUAGE_ENGLISH || SELECTED_LANGUAGE == LANGUAGE_PHILIPPINES
	// ÁøÇà»óÅÂ
	g_pRenderText->RenderText(m_iPos_x+98, m_iPos_y - m_iHeight - 12, GlobalText[1529] );	// 1529 "»óÅÂ"
	// µî·Ï¼ø¼­
	g_pRenderText->RenderText(m_iPos_x+123, m_iPos_y - m_iHeight - 12, GlobalText[1530] );	// 1530 "µî·Ï¼ø"
}
...
BOOL CUIBCDeclareGuildListBox::RenderDataLine(int iLineNumber)
{
	...
	if( m_TextListIter->byIsGiveUp )
		sprintf(Text,"%s", GlobalText[1531] );	// 1531 "Æ÷±â"
	else
		sprintf(Text,"%s", GlobalText[1532] );	// 1532 "ÁøÇàÁß"
...
CUIBCGuildListBox::CUIBCGuildListBox()
{
	m_iMaxLineCount = UIMAX_TEXT_LINE;
	m_iCurrentRenderEndLine = 0;
	m_iNumRenderLine = 15;	// 3ÀÇ ¹è¼ö·Î -_-
...
void CUIBCGuildListBox::RenderInterface()
{
	...
	g_pRenderText->RenderText(m_iPos_x+5, m_iPos_y - m_iHeight - 12, GlobalText[182] );
	// Áø¿µ
	g_pRenderText->RenderText(m_iPos_x+80, m_iPos_y - m_iHeight - 12, GlobalText[1603] );	// 1603 "Áø¿µ"
	// ÁÖÃ¼
	g_pRenderText->RenderText(m_iPos_x+120, m_iPos_y - m_iHeight - 12, GlobalText[1604] );	// 1604 "ÁÖÃ¼"

	g_pRenderText->RenderText(m_iPos_x + 18, m_iPos_y + 31 - 1, GlobalText[1977] );	// 1977 "Á¡¼ö"
}
...
BOOL CUIBCGuildListBox::RenderDataLine(int iLineNumber)
{
	...
	// Áø¿µ
	if( m_TextListIter->byJoinSide == 1 )
		sprintf(Text,"%s", GlobalText[1606] );	// 1606 "¼ö¼ºÃø"
	else
		sprintf(Text,"%s", GlobalText[1605] );	// 1605 "°ø¼ºÃø"
	g_pRenderText->RenderText(iPos_x + 100, iPos_y, Text, 0, 0, RT3_WRITE_RIGHT_TO_LEFT);

	// ÁÖÃ¼
	if( m_TextListIter->byGuildInvolved == 1 )
		sprintf(Text,"%s", GlobalText[1607] );	// 1607 "ÁÖÃ¼"
	else
		sprintf(Text,"%s", GlobalText[1608] );	// 1608 "º¸Á¶"
	...
	char Dummy[300];	if (Select_Guild == iLineNumber)
	{
		...
		g_pRenderText->RenderText(m_iPos_x + 60, m_iPos_y + 31 - 1, Dummy );	// 1977 "Á¡¼ö"
	}
...
CUIMoveCommandListBox::CUIMoveCommandListBox()
{
	...
	m_iNumRenderLine = 21;	// 3ÀÇ ¹è¼ö·Î -_-
...
void CUICurQuestListBox::AddText(DWORD dwQuestIndex, const char* pszText)
{
	...
	m_TextList.push_front(sCurQuestItem);	// µÚºÎÅÍ ·£´õÇÏ±â ¶§¹®¿¡ push_front »ç¿ë.

	SendUIMessageDirect(UI_MESSAGE_LISTSCRLTOP, 0, 0);	// ½ºÅ©·Ñ¹Ù ½æ À§Ä¡¸¦ ¸Ç À§·Î.
	SLSetSelectLine(0);									// ¼±ÅÃ ¾øÀ½.
}
...
void CUICurQuestListBox::RenderInterface()
{
	...
	// ·»´õµÇ´Â ÁÙ ¼ö º¸´Ù ¸®½ºÆ® ÁÙ ¼ö°¡ ÀÛ°Å³ª °°À¸¸é ½ºÅ©·Ñ¹Ù¸¦ ¾È±×¸².
	if (GetLineNum() <= m_iNumRenderLine)
		return;
...
BOOL CUICurQuestListBox::DoLineMouseAction(int iLineNumber)
{
	if (::CheckMouseIn(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13))
	{
		if (MouseLButtonPush)	// MouseLButtonDBClick
		{
			...
			// ¼±ÅÃÇÑ ÁÙÀÌ ÀÌÀü¿¡ ¼±ÅÃÇÑ ÁÙÀÌ¶ó¸é ¸®ÅÏ.
			...
			// ±âÅ¸»óÈ²¿¡ ÀÇÇÑ Äù½ºÆ®¶ó¸é Äù½ºÆ® ¿­±â ¹öÆ° È°¼ºÈ­, ¾Æ´Ï¶ó¸é ºñÈ°¼ºÈ­.
...
void CUIQuestContentsListBox::AddText(HFONT hFont, DWORD dwColor, int nSort, const char* pszText)
{
	...
	m_TextList.push_front(sQuestContents);	// µÚºÎÅÍ ·£´õÇÏ±â ¶§¹®¿¡ push_front »ç¿ë.
...
void CUIQuestContentsListBox::RenderInterface()
{
	...
	// ·»´õµÇ´Â ÁÙ ¼ö º¸´Ù ¸®½ºÆ® ÁÙ ¼ö°¡ Å©¸é ½ºÅ©·Ñ¹Ù¸¦ ±×¸².
	if (GetLineNum() > m_iNumRenderLine)
	{
		g_pGuardWindow->RenderScrollBarFrame(m_iPos_x + m_iWidth - 8, m_fScrollBarRange_top,
			m_fScrollBarRange_bottom - m_fScrollBarRange_top);
...
// ¸®½ºÆ® ¹Ú½º ·£´õ Áß °¡Àå ³ªÁß¿¡ È£Ãâ.
void CUIQuestContentsListBox::RenderCoveredInterface()
{
// ¼±ÅÃÇÑ Ç×¸ñÀÌ ÀÖÀ¸¸é ¾ÆÀÌÅÛ ¶Ç´Â ½ºÅ³ ÅøÆÁ ·£´õ.
	m_TextListIter = SLGetSelectLine();
...
// DoLineMouseAction() ÇÔ¼ö È£ÃâÀü¿¡ È£Ãâ µÊ.
void CUIQuestContentsListBox::DoActionSub(BOOL bMessageOnly)
{
#ifdef KWAK_FIX_COMPILE_LEVEL4_WARNING
	bMessageOnly = FALSE;
	SLSetSelectLine(bMessageOnly);	// ¸¶¿ì½º Ä¿¼­°¡ ¾ÆÀÌÅÛ, ½ºÅ³ ÅØ½ºÆ® Ç×¸ñ À§¿¡ ¾øÀ» ¶§ ÅøÆÁÀ» ¾È±×¸®±â À§ÇÔ.
#else // KWAK_FIX_COMPILE_LEVEL4_WARNING
	SLSetSelectLine(0);	// ¸¶¿ì½º Ä¿¼­°¡ ¾ÆÀÌÅÛ, ½ºÅ³ ÅØ½ºÆ® Ç×¸ñ À§¿¡ ¾øÀ» ¶§ ÅøÆÁÀ» ¾È±×¸®±â À§ÇÔ.
#endif // KWAK_FIX_COMPILE_LEVEL4_WARNING
}

BOOL CUIQuestContentsListBox::DoLineMouseAction(int iLineNumber)
{
	// °¢ Ç×¸ñ¿¡ ¸¶¿ì½º Ä¿¼­°¡ ÀÖÀ¸¸é Ç×¸ñ ¼±ÅÃ.
	if (::CheckMouseIn(m_iPos_x, GetRenderLinePos_y(iLineNumber) - 3, m_iWidth - m_fScrollBarWidth + 1, 13))
	{		
		int nSelLine = m_iCurrentRenderEndLine + iLineNumber + 1;
		SLSetSelectLine(nSelLine);
...
void CUIPackCheckBuyingListBox::AddText(IGS_SelectBuyItem& _Item)
{
	...
	// Å¬·¡½º »ý¼º½Ã¿¡ staticº¯¼ö·Î °¹¼ö¸¦ ÀÎµ¦½º·Î »ç¿ëÇÑ´Ù
	// ¸®½ºÆ® ¹Ú½ºÀÇ ¼±ÅÃÀ» µû¶ó ÀÌµ¿ÇÏ±â ¶§¹®¿¡ »ç¿ëÇÒ ÇÊ¿ä ¾øÀ½
...
BOOL CUIPackCheckBuyingListBox::RenderDataLine(int nLine)
{
	...
	{
 		// Å¬·¡½ºÀÇ ÀÎµ¦½º¿Í Ã¼Å©µÈ ÀÎµ¦½º¸¦ ºñ±³ ¾Æ´Ï¶ó¸é false
// 		if(m_TextListIter->m_RadioBtn.GetRadioBtnIsEnable() != CRadioButton::m_nIterIndex)
// 			m_TextListIter->m_RadioBtn.SetCheckState(false);

		if(SLGetSelectLineNum() == m_iCurrentRenderEndLine + nLine + 1)		//¼±ÅÃµÈ ¸®½ºÆ®´Â ¹«Á¶°Ç Ã¼Å©»óÅÂ·Î ¸¸µç´Ù
...
{
	// Å¬·¡½ºÀÇ ÀÎµ¦½º Â÷ÈÄ È°¼ºÈ­µÈ Å¬·¡½º ÀÎµ¦½º¶û ºñ±³
...
bool CRadioButton::UpdateMouseAction()
{
	...
	else if(SEASON3B::IsRelease(VK_LBUTTON) && _Temp)
	{
		...
		// Å¬·¡½ºÀÇ ÀÎµ¦½º ¹øÈ£¸¦ staticº¯¼ö¿¡ ±â¾ï½ÃÅ²´Ù
	...
	{
		// ´©¸¥ »óÅÂ¿¡¼­ ¿ÜºÎ·Î Æ÷Ä¿½º¸¦ °®°í °¬À»¶§ °ú°Å »óÅÂ°ªÀ¸·Î µ¹¸°´Ù
		m_byMouseState = LBTN_UP;
		return false;
	}
	return false;
...
{
	//Ã¼Å© ¹Ú½ºÀÇ ¼±ÅÃ¿¡ µû¶ó ¶óµð¿À ¹öÆ°µµ ¼±ÅÃÀ» µû¶ó°¡°Ô ÇÑ´Ù
	m_bCheckState = _nState;

	if(m_bCheckState)
		m_byMouseState = LBTN_UP;		//¸®½ºÆ® ¹Ú½ºÀÇ ¼±ÅÃ¿¡ µû¶ó°¡±â À§ÇØ ¹«Á¶°Ç Ã¼Å©
	else
		m_byMouseState = LBTN_DEFAULT;	//¼±ÅÃµÇÁö ¾ÊÀº °ÍÀº ¼±ÅÃµÇÁö ¾ÊÀº ÀÌ¹ÌÁö Ãâ·ÂÅä·Ï
