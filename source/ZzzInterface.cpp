...
#else // KWAK_FIX_KEY_STATE_RUNTIME_ERR
	if( HIBYTE(GetAsyncKeyState(VK_CONTROL))==128 && HIBYTE(GetAsyncKeyState(VK_CAPITAL))==128 )
#endif // KWAK_FIX_KEY_STATE_RUNTIME_ERR
	{
...
#else // KWAK_FIX_KEY_STATE_RUNTIME_ERR
		if(HIBYTE(GetAsyncKeyState(VK_CAPITAL))==128)
		{
			//Art키를 눌렀을때..
			return false;
		}
...
#else // KWAK_FIX_KEY_STATE_RUNTIME_ERR
		&& HIBYTE(GetAsyncKeyState(VK_CAPITAL))!=128)
#endif // KWAK_FIX_KEY_STATE_RUNTIME_ERR
#else	// ASG_ADD_GENS_SYSTEM
#ifdef KWAK_FIX_KEY_STATE_RUNTIME_ERR
...
bool CheckCommand(char *Text, bool bMacroText )
{
    ...
    if( bMacroText==false && LogOut == false )
    {
		...
		{
			char Text[32];
			//int oldVal = g_shCameraLevel++;
			//g_shCameraLevel++;
			//sprintf(Text, "g_shCameraLevel: %d => %d", oldVal, g_shCameraLevel);
			//g_pChatListBox->AddText("", Text, SEASON3B::TYPE_SYSTEM_MESSAGE);              
			return false;
		}
		if(strcmp(Text, "/check2")==0)
		{
			char Text[32];
			//int oldVal = g_shCameraLevel--;
			//g_shCameraLevel--;
...
#else // KWAK_FIX_KEY_STATE_RUNTIME_ERR
					if( HIBYTE(GetAsyncKeyState(VK_CONTROL)) && HIBYTE(GetAsyncKeyState(VK_CAPITAL)) )//&& PartyNumber>0 )
#endif // KWAK_FIX_KEY_STATE_RUNTIME_ERR
					{
						if ( FindText( Hero->ID, "webzen" ) )
						{
...
void MoveInterface()
{
	...
	if(MacroTime == 0)
	{
        ...
        if ( InChaosCastle()==false )
        {
			if(HIBYTE(GetAsyncKeyState(VK_CAPITAL)))	// ALT 키를 누르고
			{
				for(int i=0;i<9;i++)
				{
